const c="/assets/Blog-post01-00-BbZ3Z1hc.webp",d=`---
...
`,u=Object.assign({"../assets/blogImages/Blog-post01-00.webp":c}),p=Object.fromEntries(Object.entries(u).map(([e,n])=>[e.split("/").pop()??"",n]));function g(e){const n=e.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);if(!n)return{data:{},content:e};const[,o,i]=n,s={};return o.split(`
`).forEach(a=>{const r=a.indexOf(":");if(r===-1)return;const l=a.slice(0,r).trim();let t=a.slice(r+1).trim();typeof t=="string"&&t.startsWith("[")&&t.endsWith("]")?t=t.slice(1,-1).split(",").map(h=>h.trim()).filter(Boolean):t==="true"?t=!0:t==="false"&&(t=!1),s[l]=t}),{data:s,content:i.trim()}}const y=Object.assign({"../blogPosts/00-Obsidian-as-my-Second-Brain.md":d}),m=Object.entries(y).map(([e,n])=>{const{data:o,content:i}=g(n);return{slug:e.split("/").pop()?.replace(".md","")??"",...o,content:i}}).filter(e=>e.published).sort((e,n)=>new Date(n.date).getTime()-new Date(e.date).getTime());export{p as i,m as p};
